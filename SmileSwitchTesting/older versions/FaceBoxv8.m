...
mean_mouth_r = 0.4275;
mean_mouth_g = 0.0755;
stdev_mouth_r = 0.0372;
stdev_mouth_g = 0.0187;
...
for i=1:k
    ...
    for y=1:rows
        for x=1:columns
            ...
            mouth_yes_no=exp(-((NormalizedRed-mean_mouth_r)^2)/(2*(stdev_mouth_r^2))-((NormalizedGreen-mean_mouth_g)^2)/(2*(stdev_mouth_g^2)));
            CFM(y,x) = skin_yes_no;
            CFM_mouth(y,x) = mouth_yes_no;
...
%     axis([0 frame_y 0 frame_x])
    ...
    figure;imshow(CFM);
    figure;imshow(CFM_mouth);
    title (['MOUTH',int2str(i)]);
 ...
 %   figure; imshow(CFM);
  %  title (['CFM Image',int2str(i)]);
    ...
    redThreshold = 0.4275; 
    greenThreshold = 0.2755;
    blueThreshold = 0.2969;
