...
%     axis([0 240 0 320])
    ...
    croppedCFM = CFM(y_1:y_2,x_1:x_2,:);
...
y_Centroid = Centroid(2);
    ...
    if y_Centroid<10
     ...
     mouthImage = croppedImage(y_1:y_2,x_1:x_2,:);
     mouthCFM = croppedCFM(y_1:y_2,x_1:x_2,:);
     
   figure; imshow(mouthImage);
     title (['Mouth Image',int2str(i)]);
        figure; imshow(mouthCFM);
     title (['Mouth CFM',int2str(i)]);
...
%     
     white = 0;
     [rows, columns, numberOfColorPlanes] = size(mouthImage);
     
     for y=1:rows
         for x=1:columns
             if (mouthCFM(y,x,1)<0.1)
                 white(y,x)=1;
             end
         end
    end
